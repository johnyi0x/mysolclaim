import type { FeeConfig } from "./fee-config";
import {
  DEFAULT_FEE_PERCENT,
...
/** FAQ copy uses live FEE_PERCENT / REFERRAL_SHARE_PERCENT — never hardcode %. */
export function getFaqItems(config?: Partial<FeeConfig>): FaqItem[] {
  const feePercent = config?.feePercent ?? DEFAULT_FEE_PERCENT;
  const referralShare =
    config?.referralSharePercent ?? DEFAULT_REFERRAL_SHARE_PERCENT;
  const platformShare = 100 - referralShare;
  ...
  return [
    ...
    },
    {
      ...
      answer: `We keep ${feePercent}% of the reclaimed SOL (shown before you sign). The fee is a simple transfer inside the same transaction that closes your accounts or claims cashback, so it is atomic: either you receive your SOL and we receive the fee, or nothing happens at all. You never pay out of pocket, and you can verify every fee our wallet has ever received on Solscan — the address is in the footer. If you arrived via a referral link, that same ${feePercent}% fee is split on-chain between the platform and the referrer — you still pay the same total percentage.`,
    },
    {
      question: "How does the referral program work?",
      answer: `Connect on the Referral page to get a link like mysolclaim.com/?ref=YOUR_WALLET. When someone claims after using your link, they still pay the normal ${feePercent}% service fee; ${referralShare}% of that fee (a ${platformShare}/${referralShare} split with the platform) is tipped to your wallet in the same transaction. Keep about 0.001 SOL in your wallet so Solana can land those tips. Details and your cumulative on-chain earnings are on /referral.`,
    },
    {
    ...
    },
    {
      ...
      answer: `Your wallet simulates every transaction before you approve it. Phantom, Solflare and Backpack all display the net SOL you will receive. If a transaction would do anything other than reclaim SOL/USDC and pay the disclosed ${feePercent}% fee, the simulation would show it.`,
    },
    {
    ...
    },
    {
      ...
      answer:
        "No. Scanning and transaction building happen entirely in your browser, talking directly to the Solana blockchain. Our only server code is read-only (recent claims, fee config, optional referral bind). There is nothing that can move your funds without your signature.",
    },
  ];
}

/** @deprecated Prefer getFaqItems(useFeeConfig()) so % tracks env. */
export const FAQ_ITEMS: FaqItem[] = getFaqItems();
